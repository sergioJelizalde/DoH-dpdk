...
static void layer_forward_avx2(const float *W, const float *B,
    ...
    for (; j + 8 <= size_out; j += 8) {
        ...
        for (int k = 0; k < size_in; k++) {
            __m256 weight_vec = _mm256_loadu_ps(&W[k * size_out + j]);
            __m256 input_broadcast = _mm256_set1_ps(in[k]);
            acc = _mm256_fmadd_ps(input_broadcast, weight_vec, acc);
        }
        
        if (!is_output) {
            __m256 zero = _mm256_setzero_ps();
            acc = _mm256_max_ps(acc, zero); // ReLU
        }
        
        _mm256_storeu_ps(&out[j], acc);
    }
    
    // Process 4 elements at a time with SSE (if needed)
    for (; j + 4 <= size_out; j += 4) {
        __m128 acc = _mm_loadu_ps(&B[j]);
        
        for (int k = 0; k < size_in; k++) {
            __m128 weight_vec = _mm_loadu_ps(&W[k * size_out + j]);
            __m128 input_broadcast = _mm_set1_ps(in[k]);
            acc = _mm_fmadd_ps(input_broadcast, weight_vec, acc);
        }
        
        if (!is_output) {
            __m128 zero = _mm_setzero_ps();
            acc = _mm_max_ps(acc, zero); // ReLU
        }
...
/* run MLP prediction - same as before but using AVX2 version */
static int predict_mlp(const float *in_features, float *buf_a, float *buf_b) {
    ...
    for (int L = 0; L < NUM_LAYERS; L++) {
        ...
        layer_forward_avx2(WEIGHTS[L], BIASES[L],
                                   in_buf, out_buf,
                                   LAYER_SIZES[L],
                                   LAYER_SIZES[L+1],
                                   is_output_layer);
        // ... rest of the function remains the same as your original version
        if (is_output_layer) {
            #if IS_BINARY_CLASSIFICATION
            for (int i = 0; i < LAYER_SIZES[L+1]; i++)
                out_buf[i] = fast_sigmoid(out_buf[i]);
